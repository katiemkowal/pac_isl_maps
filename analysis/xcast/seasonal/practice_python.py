...
for t, initial_month_name in enumerate(initial_month_names):
    ...
    for l, lead in enumerate(np.unique(hindcast_data.L.values)):
        ...
        hindcasts_prob = xc.gaussian_smooth(cca_hcsts_prob.isel(L=l), kernel=3)
        ...
        grocs_cca.append(xc.GROCS(hindcasts_prob, T))
    ...
    regions = [pacislands_coordinates, fiji_coordinates, kiribati_coordinates, solomon_coordinates, png_coordinates]
    region_names = ['Pacific Islands', 'Fiji', 'Kiribati', 'Solomon Islands', 'Papua New Guinea']
    
    ###### PEARSONS PLOTS

    # Assuming you have data for models and seasons
    models = np.unique(pearsons.M.values)
    seasons = target_seas[t].copy()
