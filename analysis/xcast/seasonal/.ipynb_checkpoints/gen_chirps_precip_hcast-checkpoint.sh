...
'close 1'
'set gxout fwrite'
'set fwrite chirps_ld${ld}.dat'
i=1991
while(i<=2022)
'open /cpc/int_desk/endalk/chirps/chirps_daily.ctl'
'set lat -50 50'
'set lon -180 180'
yr = i
yrr = i
...
'set x 1 721'
'set gxout fwrite'
'set fwrite chirps_hind_precip_ld${ld}.dat'
...
lat[:] = lats + 0.5*np.arange(nlat)
lon[:] = lonw + 0.5*np.arange(nlon)
